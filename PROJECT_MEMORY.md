...
- 2026-09-08T18:34+08:00

## Objective

- VERIFIED: 利用原代人 CD4 T 细胞全基因组 Perturb-seq，评估多个单步扰动执行器的可组合性，并构建不确定性感知的短程序贯扰动规划器；最终目标是提出并实验验证 Th2→Th17 的顺序性 CRISPR 干扰方案。
- VERIFIED: 当前已完成正式训练前的合同修正、64扰动试点和Scratch/Transfer正式架构20步冒烟；种子20260901和20260902的Scratch/Transfer全量训练已完成，种子20260903的Scratch已完成，Transfer正在进行。
...
- VERIFIED: 2026-09-07性能诊断确认正式训练的主要瓶颈在主机端批次组装：20秒显卡采样平均利用率约32%，训练主线程约83–100% CPU，期间无持续磁盘读取；检查点写入不是持续低利用率的主因。
- VERIFIED: 2026-09-07加入保持批次顺序、随机数顺序和合同哈希不变的后台批次预取；正式入口改为CPU固定内存组装、训练循环异步送入CUDA。e3_pipeline为41项通过、5项因无PyTorch跳过；e3_state合成训练冒烟和CUDA传输断言通过。该优化尚未用于正在运行的Scratch进程。
- VERIFIED: 2026-09-07种子20260902 Scratch已从20,500步检查点恢复并完成40,000步，最佳验证MMD为0.01166546458（第37,000步）；结果已下载至`research/state_d2/training_result_scratch_20260902.json`，测试集仍封存。
...
- ACTIVE: 2026-09-08种子20260903 Transfer已由监控器自动启动（远端进程10861），当前仍处于数据缓存初始化阶段，尚无检查点；测试集仍封存。
- PENDING: 尚未完成种子20260903 Transfer、简单基线对比和D2响应评价；当前不能据此给出最终Transfer或Scratch模型结论。`train_two_phase`与`D2BatchStream`已提供公平两阶段训练接口，不能把验证MMD直接当作测试性能。
...
- 2026-09-07T17:56+08:00: 对正式训练低显卡利用率完成基线诊断，定位为同步主机端CSR批次组装；加入有序后台预取、固定内存和非阻塞设备传输，未改变模型合同、数据顺序或测试封存。远端e3_pipeline测试为41项通过、5项跳过，e3_state合成训练/ CUDA传输冒烟通过；当前种子20260902 Scratch继续使用旧代码完成，后续Transfer和种子20260903使用优化入口。

- 2026-09-07T18:22+08:00: 在已保存的20,500步`last.ckpt`上安全恢复种子20260902 Scratch；合同哈希一致。因恢复需要确定性重放已完成批次，启动阶段暂时只占用CPU；旧监控器已停止，新监控器进程107196已等待优化版Scratch完成后串联Transfer。

- 2026-09-07T23:05+08:00: 种子20260902 Scratch完成40,000步，最佳验证MMD为0.01166546458（第37,000步）；Transfer已自动启动并完成至少1,000步，当前尚未读取测试集。

- 2026-09-08T14:57+08:00: 种子20260902 Transfer完成33,000步并按验证MMD早停，最佳验证MMD为0.01279209289（第29,000步）；Scratch/Transfer结果均已下载，尚未读取测试集，下一步启动种子20260903 Scratch。

- 2026-09-08T15:02+08:00: 启动种子20260903 Scratch（进程10516），设置完成后自动串联Transfer（监控器10861）；测试集仍封存。

- 2026-09-08T18:34+08:00: 种子20260903 Scratch完成31,000步并早停，最佳验证MMD为0.01192633060（第27,000步）；Transfer已自动启动，测试集仍封存。
