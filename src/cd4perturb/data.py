...
def _require_donor_paths(files: Iterable[Path], donor_id: str) -> str:
    donor_id = str(donor_id).upper()
    if donor_id not in {"D1", "D2", "D3", "D4"}:
        raise ValueError(f"unsupported donor_id: {donor_id}")
    paths = list(files)
    if not paths or any(not path.name.startswith(f"{donor_id}_") for path in paths):
        raise ValueError(f"paths must contain only {donor_id} files ({donor_id} files only)")
    return donor_id
...
def _read_csr_block_columns_h5(handle, start: int, stop: int,
                               columns: list[int], n_vars: int,
                               csr_patch: Mapping | None = None) -> np.ndarray:
    ...
    indptr = _read_indptr_slice_h5(handle, start, stop + 1, csr_patch)
...
def _validate_patch_metadata(patch: Mapping, original_path: str | Path) -> dict[int, np.ndarray]:
    """Load and verify the small, explicit X/indptr sidecar overlay.

    The sidecar is deliberately strict: it can name only D1_Rest, only the
    official public object, and only the two pre-registered 1001-element
    chunks.  Bytes are checked before they are decoded as little-endian int64.
    """
    if str(patch.get("version")) != "d1_rest_csr_patch.v1":
        raise ValueError("unsupported CSR patch version")
    original = str(Path(original_path).resolve())
    declared = str(patch.get("original_path", ""))
    if declared != original:
        raise ValueError(f"CSR patch original_path mismatch: {declared!r} != {original!r}")
    if "D2" in declared.upper() or "D2" in str(patch.get("dataset_id", "")).upper():
        raise ValueError("D2 paths are forbidden for CSR patches")
    if str(patch.get("dataset_id")) != "D1_Rest" or str(patch.get("hdf5_dataset")) != "X/indptr":
        raise ValueError("CSR patch is not the declared D1_Rest/X/indptr patch")
    parsed = urlparse(str(patch.get("source_url", "")))
    if parsed.scheme != "https" or parsed.netloc != "genome-scale-tcell-perturb-seq.s3.amazonaws.com" or parsed.query or parsed.fragment or parsed.username or parsed.password:
        raise ValueError("CSR patch source_url is not the credential-free official HTTPS object")
    if not parsed.path.endswith("/D1_Rest.assigned_guide.h5ad"):
        raise ValueError("CSR patch source_url does not name D1_Rest")
    if patch.get("d2_responses_used") is not False:
        raise ValueError("CSR patch has d2_responses_used=true or missing")
    records = patch.get("chunks")
    if not isinstance(records, list) or len(records) != 2:
        raise ValueError("CSR patch must contain exactly two chunks")
    overlays: dict[int, np.ndarray] = {}
    expected = {
        (450450, 451450, 20762308416, 20762316423),
        (451451, 452451, 20810454136, 20810462143),
    }
    root = Path(str(patch.get("metadata_root", ""))) if patch.get("metadata_root") else None
    if root is None:
        raise ValueError("CSR patch metadata_root is required")
    for row in records:
        if not isinstance(row, Mapping):
            raise ValueError("CSR patch chunk record is not an object")
        key = (int(row.get("row_start", -1)), int(row.get("row_end", -2)),
               int(row.get("byte_start", -1)), int(row.get("byte_end", -2)))
        if key not in expected:
            raise ValueError(f"undeclared CSR patch range: {key}")
        if bool(row.get("row_end_inclusive", True)) is not True:
            raise ValueError("CSR patch row ranges must be inclusive")
        row_count = key[1] - key[0] + 1
        byte_count = key[3] - key[2] + 1
        if row_count != 1001 or byte_count != 8008 or byte_count != row_count * 8:
            raise ValueError("CSR patch chunk length is not 1001 int64 values")
        rel = str(row.get("patch_file", ""))
        if not rel or Path(rel).is_absolute() or ".." in Path(rel).parts:
            raise ValueError("CSR patch file must be a relative path below metadata_root")
        path = (root / rel).resolve()
        if root.resolve() not in path.parents:
            raise ValueError("CSR patch file escapes metadata_root")
        raw = path.read_bytes()
        if len(raw) != byte_count:
            raise ValueError(f"CSR patch byte count mismatch for {rel}")
        digest = hashlib.sha256(raw).hexdigest()
        if digest != str(row.get("sha256")):
            raise ValueError(f"CSR patch hash mismatch for {rel}")
        values = np.frombuffer(raw, dtype="<i8").copy()
        if len(values) != row_count or np.any(values < 0):
            raise ValueError(f"CSR patch pointer payload invalid for {rel}")
        if np.any(np.diff(values) < 0):
            raise ValueError(f"CSR patch pointer payload is decreasing for {rel}")
        if key[0] in overlays:
            raise ValueError("duplicate CSR patch row range")
        overlays[key[0]] = values
    if set((start, start + len(values) - 1) for start, values in overlays.items()) != {(450450, 451450), (451451, 452451)}:
        raise ValueError("CSR patch does not cover exactly the two declared chunks")
    return overlays


def _load_csr_patch(path: str | Path, original_path: str | Path) -> tuple[dict[int, np.ndarray], dict]:
    """Read a patch JSON and verify its sidecar bytes before any CSR access."""
    target = Path(path)
    payload = json.loads(target.read_text(encoding="utf-8"))
    # Resolve patch_file relative to the sidecar's own directory.  The
    # producer may record an absolute provenance root, but consumers must not
    # follow that path on a different machine.
    payload["metadata_root"] = str(target.parent)
    overlays = _validate_patch_metadata(payload, original_path)
    return overlays, payload


def _read_indptr_slice_h5(handle, start: int, stop: int,
                          csr_patch: Mapping | None = None) -> np.ndarray:
    """Read an indptr slice and overlay only declared patch chunks."""
    x = handle["X"]
    raw = np.asarray(x["indptr"][start:stop], dtype=np.int64)
    if csr_patch is None:
        return raw
    # A loaded overlay maps global row starts to exactly 1001 pointer values.
    for row_start, values in csr_patch.items():
        row_stop = row_start + len(values)
        left = max(start, row_start)
        right = min(stop, row_stop)
        if left < right:
            raw[left - start:right - start] = values[left - row_start:right - row_start]
    return raw


def _validate_csr_structure_h5(handle, csr_patch: Mapping | None = None) -> dict:
    ...
    if isinstance(x, h5py.Dataset):
        return
    pointers = _read_indptr_slice_h5(handle, 0, int(x["indptr"].shape[0]), csr_patch)
    valid = bool(len(pointers) and pointers[0] == 0 and not np.any(np.diff(pointers) < 0) and
                 int(pointers[-1]) == int(x["data"].shape[0]))
    if not valid:
        raise ValueError("invalid CSR indptr/data bounds; input H5AD is corrupt or truncated")
    return {"indptr_length": int(len(pointers)), "data_length": int(x["data"].shape[0]),
            "first_pointer": int(pointers[0]), "last_pointer": int(pointers[-1])}
...
def summarize_d1_candidates(paths: Iterable[str | Path], sample_size: int = 2048,
                            ...
                            seed: int = 20260901, donor_id: str = "D1") -> list[dict]:
    """Build a target-direction-blind candidate table from one donor.
    ...
    _require_donor_paths(files, donor_id)
    for path in files:
...
def summarize_candidates(paths: Iterable[str | Path], sample_size: int = 2048,
                         min_cells_per_guide: int = 100, min_guides_per_gene: int = 2,
                         seed: int = 20260901, donor_id: str = "D2") -> list[dict]:
    """Donor-generic candidate API; response-blind by construction."""
    return summarize_d1_candidates(paths, sample_size, min_cells_per_guide,
                                   min_guides_per_gene, seed, donor_id=donor_id)


def freeze_shared_gene_order(paths: Iterable[str | Path], pilot_genes: Iterable[str] = (),
                             n_genes: int = 2000, sample_size: int = 4096,
                             seed: int = 20260901, block_rows: int = 2048,
                             donor_id: str = "D1") -> dict:
    """Freeze a shared feature order from one donor's NTC cells only.
    ...
    files = [Path(p) for p in paths]
    donor_id = _require_donor_paths(files, donor_id)
    ...
    payload = {"version": "shared_gene_order.v1", "gene_order": selected_ids,
               ...
               "algorithm": f"{donor_id} NTC log1p(CP10K) mean-variance ranking; forced pilot genes first",
               ...
               "seed": int(seed), "development_donor": donor_id,
               "ntc_donors_used": [donor_id], "response_donors_used": []}
    if donor_id == "D1":
...
def build_d1_ntc_latent(paths: Iterable[str | Path], gene_order: Iterable[str],
                        *, sample_per_condition: int = 10000, n_components: int = 50,
                        seed: int = 20260901, block_rows: int = 2048,
                        donor_id: str = "D1") -> tuple[np.ndarray, np.ndarray, dict]:
    """Create a fixed PCA latent space from one donor's NTC sample.
    ...
    if len(matrix) < 2:
        raise ValueError(f"fewer than two {donor_id} NTC cells available for latent space")
    ...
    metadata = {"version": "d1_ntc_pca_latent.v1", "dimensions": components,
                ...
                "development_donor": donor_id, "ntc_donors_used": [donor_id],
                "response_donors_used": []}
    if donor_id == "D1":
        metadata["d2_responses_used"] = False
...
def build_d1_effect_matrix(paths: Iterable[str | Path], genes: Iterable[str],
                           ...
                           csr_patch: str | Path | None = None,
                           progress_path: str | Path | None = None,
                           donor_id: str = "D1") -> dict:
    """Build condition-matched guide and robust gene effects for one donor.
    ...
    files = [Path(p) for p in paths]
    donor_id = _require_donor_paths(files, donor_id)
    if guide_qc is not None and guide_qc.get("development_donor", donor_id) != donor_id:
        raise ValueError("guide QC donor does not match effect-matrix donor")
    patch_payload = None
    patch_overlays: dict[int, np.ndarray] | None = None
    if csr_patch is not None:
        if donor_id != "D1" or len(files) != 3 or not any(p.name.startswith("D1_Rest.") for p in files):
            raise ValueError("CSR patch is valid only for the complete D1 Rest/Stim8hr/Stim48hr set")
        rest = next(p for p in files if p.name.startswith("D1_Rest."))
        patch_overlays, patch_payload = _load_csr_patch(csr_patch, rest)
    ...
    _progress("run_started", conditions=[p.name.split(".")[0] for p in files],
              genes=len(gene_order), block_rows=int(block_rows),
              development_donor=donor_id, response_donors_used=[donor_id])
    ...
    for path in files:
        ...
        with h5py.File(path, "r") as handle:
            ...
            x_node = handle["X"]
            overlays = patch_overlays if path.name.startswith("D1_Rest.") else None
            _validate_csr_structure_h5(handle, overlays)
            ...
            for start in range(0, n_cells, step):
                ...
                block = _read_csr_block_columns_h5(handle, start, stop, col_idx, n_vars, overlays)
    ...
    payload = {
        "version": "d1_effect_matrix.v2" if donor_id == "D1" else "effect_matrix.v3",
        ...
        "development_donor": donor_id,
        "response_donors_used": [donor_id],
    }
    payload["csr_integrity"] = {
        "raw_input_modified": False,
        "csr_patch_applied": bool(csr_patch is not None),
        "patch_sidecar": str(csr_patch) if csr_patch is not None else None,
        "patch_version": patch_payload.get("version") if patch_payload else None,
        "patch_artifact_hash": patch_payload.get("artifact_hash") if patch_payload else None,
    }
    if donor_id == "D1":
        payload["d2_responses_used"] = False
